...
namespace Planner {
    ...
    enum IPSolverType {CPLEX = 0, GUROBI = 1};
    ...
    public:
        ...
        static double calculateRHS_opt(const Operand & currOperand, vector<double> & fluents);
        ...
        public:
            ...
            double evaluateMDD(vector<double> & minFluent, vector<double> & maxFluent , const int & pneCount);
        ...
        public:
            ...
            bool isSatisfiedMDDCalculate(vector<double> & minFluents, vector<double> & maxFluents) const;
    ...
    protected:
        ...
        static list<pair<int, VAL::time_spec> > preconditionlessPropositionalActions;
        ...
        //@mpcastro : my implemenation. Saves the numerical variables that the action affects
        static vector<vector<int> > actionsToAffectedNumericalVariables;
        
        //@mpcastro : my implemenation. Save cost of each action
        static vector<double> actionCost;
    ...
    public:
        ...
        static bool useMIP;
        static IPSolverType ipSolver;
        static bool useExtraConstraints;
        static bool usePlanLength;
        static bool useStartPlanLength;
        static bool blindSearch;
        static bool useBoxing;
        static bool useValidityInequality;
        static bool useDeleteRelaxation;
        static bool useTemporalRelaxation;
        static bool useSEQConstraints;
        static bool useMDD;
        static bool useOperatorCounting;
        static bool compareMDDMIP;
        static bool useLPRelaxation;
        static bool allGoalStatesMDD;
        static string nameFileCompare;
        static int 	MDD_width;
        static int 	MDD_heuristic;
        static bool	MDD_intervalR;
        static bool	MDD_eRPG;
        static bool MDD_iterative;
        ...
        static vector<instantiatedOp*> getInstantiatedOps() { return instantiatedOps; };
        static int getNOp() {
            return instantiatedOps.size();
        };
        static Literal* getLiteral(const int & i) { return literals[i]; };
        static int getNLiterals() {
        ...
        // @mpcastro : my implementation
        static const vector<list<NumericPrecondition> > & getProcessedStartNumericPreconditions()
        {
            return actionsToProcessedStartNumericPreconditions;
        }
        ...
        // @mpcastro : my implementation --> returns the list of nuemrical effects for all the actions.
        static const vector<list<NumericEffect> > & getStartNumericEffects()
        {
            return actionsToStartNumericEffects;
        }
        
        // @mpcastro : my implementation --> returns a vector with the numerical variables that each action affects
        static const vector<vector<int> > & getAffectedNumericalVariables()
        {
            return  actionsToAffectedNumericalVariables;
        }
        
        // @mpcastro : my implementation --> returns a vector with the numerical variables that each action affects
        static const vector<double> & getActionCost()
        {
            return  actionCost;
        }
        
       
        // returns the max values needed in the problem
        static vector<double> & getMaxNeeded(){
            return maxNeeded;
        }
        ...
        static list<pair<int, VAL::time_spec> > & getPreconditionlessPropositionalActions()
        {
            return preconditionlessPropositionalActions;
        }
        ...
        static int getActionCount() { return actionsToProcessedStartRPGNumericPreconditions.size(); };
    ...
    public:
        ...
        set<int> first; // propositions that are true in the state
        vector<double> second; // values of numeric variables
        map<int, int> startedActions; // actions that are already in the plan
        map<int, int> invariants; // "over all conditions" of temporal actions ?
        map<int, int> fluentInvariants; // numeric "over all conditions" of temporal actions ?
        vector<AutomatonPosition> preferenceStatus; // for preference ?
        vector<bool> landmarkStatus; // no idea
        double prefPreconditionViolations; // for preference ?
        double cost; // actual cost to achieve a state
    ...
    protected:
        ...
        list<pair<int, VAL::time_spec> > * preconditionlessPropositionalActions;
        ...
        void updatePreferencesForFact(MinimalState & startState, const unsigned int & factID, const bool & isALiteral, const bool & polarity, const double&, double & lag, double & rpgGoalPrefViolation);
    ...
    public:
        ...
        preconditionlessPropositionalActions(pla),
        deleteArrays(b)
        {
            comparisonHeuristic = -1.0;
        ...
        //@mpcastro: structure that the getRelaxedPlan function returns
        struct EvaluationInfo {
            
            double first;       //@mpcastro: heuritistic value; -1 = the problem is infeasible
            double second;      //@mpcastro: something about the prefences
            bool goalState;     //@mpcastro: true = the current state is the goal state
        ...
        virtual EvaluationInfo getRelaxedPlan(MinimalState & theState, const double & maxPrefCost, const double & stateTS, const int & nextTIL, set<int> & goals, set<int> & goalFluents, list<pair<int, VAL::time_spec> > & helpfulActions, list<pair<double, list<ActionAndHowManyTimes > > > & relaxedPlan, map<double, list<int> > * justApplied=0, double tilFrom=0.001);
    ...
    class SubproblemMIP : public SubproblemRPG{
    public:
        SubproblemMIP (const bool & b,
                       vector<list<Literal*> > * atse,
                       vector<list<Literal*> > * atee,
                       vector<list<pair<int, VAL::time_spec> > > * eta,
                       vector<list<Literal*> > * atsne,
                       ...
                       vector<int> * iupsp,
                       vector<int> * iupsnp,
                       list<pair<int, VAL::time_spec> > * pla) : SubproblemRPG(b,atse,atee,eta,atsne, atene,neta,pta,atsp,ati,atep,atnuse, atnuee, atrnuse,atrnuee,atnusp, atnui,atnuep,atpnuep,iusp,iuip, iuep,ail,ailr,ab,abr,negail,negailr,negab,negabr, nail,nailr,nab,nabr, negnail,negnailr,negnab,negnabr, iunsp, iuni, iunep, rnp, rne, ppta, nppta,atpsp,iupsp,iupsnp,pla)
        { }
        ~SubproblemMIP () {
            if (deleteArrays) {
                assert(false);
        ...
        static double timeBuildConstraints;
        static double timeSolve;
