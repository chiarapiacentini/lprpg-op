                                                            ...
                                                            set<int> & landmarksFound, list<list<set<int> > > & disjunctiveLandmarksFound)
{
    ...
    //@mpcastro: fill first layr with facts in the initial state
    {
        ...
        if(debug){
        	...
        	for(auto i: factLayer){
        		cout << " - " << i << " " << *(RPGBuilder::getLiteral(i)) << endl; 
        	}
        }

    }
    
    //@mpcastro: iterate over actions applicable to the first state. Add the adding effects to the layer
    {
...
void LandmarksAnalysis::performLandmarksAnalysis()
{
    ...
    for (int ll = 0; ll < lCount; ++ll, ++sItr) {        
        ...
        cout << *sItr << *(RPGBuilder::getLiteral(*sItr)) << " is individual landmark fact " << ll << endl;
    }
    ...
    for (int l = lCount; efItr != efEnd; ++efItr, ++l) {
        ...
        cout << *efItr << *(RPGBuilder::getLiteral(*efItr)) << " is landmark fact " << l << endl;
    }
