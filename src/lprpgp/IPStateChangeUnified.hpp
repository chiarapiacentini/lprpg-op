...
#ifndef IPStateChangeUnified_hpp
#define IPStateChangeUnified_hpp
...
class IPStateChangeUnified : public IPStateChangeModel {
public:
    IPStateChangeUnified(MinimalState & state, int horizon, vector<bool> & aL, vector<bool> & fL, vector<bool> & aE, vector<bool> & fE, bool d = false) :  IPStateChangeModel(state,horizon, aL, fL, aE, fE, d), delta(1), infinity(10),  minMaxValues(0), minMaxSteps(0), simpleEffectActionsConstant(0), linearEffectActionEffectID(0), minEffect(0), variableActionLinear(0) {
        infinity = 100000;//
        //infinity = IloInfinity;
        FukunagaAnalysis::getActionsEffects(simpleEffectActionsConstant,linearEffectActionEffectID);
        variableActionLinear.assign(RPGBuilder::getPNECount(),vector<bool>(RPGBuilder::getNOp(),false));
...
#endif /* IPStateChangeUnified_hpp */
