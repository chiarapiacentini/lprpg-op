...
//  interestingActionUntilWeHaveNumeric = vector<set<int> >(opCount);
    ...
    #else
    {
        list<Literal*>::iterator afItr = literalGoals.begin();
        const list<Literal*>::iterator afEnd = literalGoals.end();
        // iterate over the literal goals
        for (int i = 0; afItr != afEnd; ++afItr, ++i) {
            if (LiteralAnalysis::isStatic(*afItr).first) {
            //if (LiteralAnalysis::isStatic(*afItr).first && !LiteralAnalysis::isStatic(*afItr).second) {
                continue;
            }
    ...
    
    {
        for (int pass = 0; pass < 2; ++pass) {
            ...
            for (int p = 0; p < pSize; ++p) {
                ...
                //if (LiteralAnalysis::isStatic(RPGBuilder::getLiteral(p)).first && !LiteralAnalysis::isStatic(RPGBuilder::getLiteral(p)).second) {
                    continue;
                }
    ...
    while (!data.empty()) {
        ...
        if (currPair.fact.first != -1) {
            ...
            */
            ...
            // find actions that increase or decrease a fluent
