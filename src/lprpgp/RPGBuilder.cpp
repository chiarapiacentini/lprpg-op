	...
	for (; fItr != fEnd; ++fItr) {
		...
		switch(currMathOp) {
			case RPGBuilder::NE_ADD:
				...
				//cout << "\t ADD: " << oldFront << " + " <<  RHS.front() << endl;
				RHS.front() += oldFront;
				}
				break;
			case RPGBuilder::NE_SUBTRACT:
				...
				const double oldFront = RHS.front(); RHS.pop_front();
				RHS.front() -= oldFront;
				//cout << "\t SUBTRACT: " << oldFront << " + " <<  RHS.front() << endl;
				}
				break;
			case RPGBuilder::NE_MULTIPLY:
				...
				const double oldFront = RHS.front(); RHS.pop_front();
				RHS.front() *= oldFront;
				//cout << "\t MULTIPLY: " << oldFront << " + " <<  RHS.front() << endl;
				}
				break;
...
double RPGBuilder::calculateRHS_opt(const Operand & currOperand, vector<double> & fluents) {
	double toReturn;
	
	switch(currOperand.numericOp) {
		case RPGBuilder::NE_CONSTANT:
			toReturn = currOperand.constantValue;
			break;
		case RPGBuilder::NE_FLUENT:
			toReturn = fluents[currOperand.fluentValue];
			break;
		case RPGBuilder::NE_VIOLATION:
			toReturn = 0.0;
			break;
...
double RPGBuilder::ArtificialVariable::evaluateMDD(vector<double> & minFluent, vector<double> & maxFluent , const int & pneCount) {
...
bool RPGBuilder::RPGNumericPrecondition::isSatisfiedWCalculate(vector<double> & maxFluents) const {
	...
	//cout << "with current value:" << localVal << endl;
	if (op == VAL::E_GREATER) {
		return (localVal > RHSConstant);
	} else {
		return (localVal >= RHSConstant);
...
// @mpcastro: our implementation to it correctly calculates the bounds for the MDD
bool RPGBuilder::RPGNumericPrecondition::isSatisfiedMDDCalculate(vector<double> & minFluents, vector<double> & maxFluents) const {
    ...
    // Case ; artificial variables
    var += pneCount;
    //cout << "Precondition based on artificial variable " << var << "\n";
    ArtificialVariable & av = RPGBuilder::getArtificialVariable(var);
    const double localVal = av.evaluateMDD(minFluents, maxFluents, pneCount);
    //cout << "with current value:" << localVal << endl;
    if (op == VAL::E_GREATER) {
        return (localVal > RHSConstant);
    } else {
...
RPGBuilder::NumericEffect::NumericEffect(const VAL::assign_op & opIn, const int & fIn, VAL::expression * formulaIn, VAL::FastEnvironment * f,VAL::TypeChecker * t)
    : fluentIndex(fIn), op(opIn), formula_one("0") {
    ExpressionBuilder builder(formula, f, t);
    valid = builder.buildFormula(formulaIn);
    size_formula = formula.size();
    if (size_formula == 1) formula_one = formula.front(); 
}
...
// @mpcastro : calculates a particular numerical effects of an action, given the current value of the variables
double RPGBuilder::NumericEffect::applyEffect(vector<double> & fluents) const {

	double RHS = 0.0;
	if( size_formula == 1) 	RHS = calculateRHS_opt(formula_one, fluents);
	else 					RHS = calculateRHS(formula, fluents);
...
list<pair<int, VAL::time_spec> > RPGBuilder::preconditionlessPropositionalActions;
...
vector<vector<int> > RPGBuilder::actionsToAffectedNumericalVariables;		//@mpcastro: my implementation
vector<double> RPGBuilder::actionCost;
...
void RPGBuilder::initialise(bool compilation) {
	...
	actionsToAffectedNumericalVariables = vector<vector<int> >(operatorCount, vector<int>(pneCount, 0) );
	actionCost = vector<double>(operatorCount, 1.0);
...
*/
	...
	//@mpcastro: compute cost for each operator - my code
	{
		if(!metricVars.empty() && !usePlanLength ){
		
			int id_metric = *metricVars.begin();
			vector<double> aux_vars(pneCount, 0.0);
			
			for (int i = 0; i < operatorCount; i++) {
				
				if(actionsToAffectedNumericalVariables[i][id_metric] == 1) {
					//Case 1: it has a cost saved as an effect

						for (auto& effect: actionsToStartNumericEffects[i]) {
							if (effect.fluentIndex == id_metric) {
								actionCost[i] = effect.applyEffect(aux_vars);
								break;
							}
		...
		if(GlobalSchedule::globalVerbosity & 64){
			cout << "Action costs" << endl;
			
			for (int i = 0; i < operatorCount; i++) {
				cout << "Action " << i << " " <<  *getInstantiatedOp(i) << ", cost = " << actionCost[i] << endl;
			}
		}
	}
    ...
    // @chiarap : add preconditionless actions
    for (int i = 0; i < RPGBuilder::getNOp(); ++i){
        list<Literal*> & prec = RPGBuilder::getStartPropositionalPreconditions()[i];
        if (prec.size()==0) preconditionlessPropositionalActions.push_back(make_pair(i,VAL::E_AT_START));
    }
...
pair<bool,int> RPGBuilder::isBiggerBetter(const int & var, const int & polarity) {
	...
	for (; preItr != preEnd; ++preItr) {
        ...
        //cout << "\tvariable " << var << " is in precondition : " << currPre << endl;
		...
		if (varWeight * polarity >= 0.0) {
            ...
            //cout << "\t\tnow we need to check " << actsToCheck.size() << " actions " << endl;
			for (; actItr != actEnd; ++actItr) {
                // if there are preconditions return false
				for (int pass = 0; pass < 2; ++pass) {
                ...
                // if action affecting the variable has preconditions, we are here
                //cout << "\t\tnow check " << *(RPGBuilder::getInstantiatedOp(*actItr)) << "  that does not have any other literal precondition " << endl;
                for (int pass = 0; pass < 2; ++pass) {
                    // check effect of
                    ...
                    // FOUND BUG before const list<int>::iterator effEnd = effList.begin();
					const list<int>::iterator effEnd = effList.end();
                    //cout << "\t\t\teffects of the actions " << effList.size() << endl;

					for (; effItr != effEnd; ++effItr) {
						RPGNumericEffect & currEff = rpgNumericEffects[*effItr];
                        //cout << "\t\t\t\tconsidering effect " << *effItr;
                        if (currEff.fluentIndex == var) {
                            //cout << " has the same variable" << endl;
							if (!simpleConstant) return pair<bool,int>(false,-1);
                            //cout << "\t\t\t\t\tis nott a simple constant effect (can still be dominated)" << endl;
                            ...
                            //cout << " is not irrelevant nor is a meteric variable" << endl;
                            ...
                            //cout << " is irrelevant or is a meteric variable" << endl;
                        }
					}
				}
...
void RPGBuilder::getInitialState(LiteralSet & initialState, vector<double> & initialFluents) {
    ...
    if (!cached) {
        ...
        while (factItr != factEnd) {
            //if (LiteralAnalysis::isStatic(*factItr).first && !LiteralAnalysis::isStatic(*factItr).second) {
    ...
    for(auto literal: isCache){
    	cout << " - " << literal->getStateID() << " " << *(RPGBuilder::getLiteral(literal->getStateID())) << endl;
    }
    */    
};
...
SubproblemRPG* RPGBuilder::pruneRPG(list<Literal*> & goals, list<int> & goalFluents, LiteralSet & initialState, vector<double> & initialFluents) {
    ...
    if (RPGBuilder::useMIP)  return new SubproblemMIP( false, // subproblem does not own the arrays
...
bool RPGBuilder::useMDD = false;
bool RPGBuilder::useOperatorCounting = false;
bool RPGBuilder::useDeleteRelaxation = false;
bool RPGBuilder::useExtraConstraints = false;
bool RPGBuilder::usePlanLength = false;
bool RPGBuilder::useStartPlanLength = false;
bool RPGBuilder::blindSearch = false;
bool RPGBuilder::useBoxing = true;
bool RPGBuilder::useValidityInequality = true;
bool RPGBuilder::useTemporalRelaxation = false;
bool RPGBuilder::useSEQConstraints = false;
bool RPGBuilder::useMIP = false;
IPSolverType RPGBuilder::ipSolver = CPLEX;
bool RPGBuilder::compareMDDMIP = false;
bool RPGBuilder::useLPRelaxation = false;
bool RPGBuilder::allGoalStatesMDD = false;
string RPGBuilder::nameFileCompare = "t.csv";
int  RPGBuilder::MDD_width = 1;
bool RPGBuilder::MDD_intervalR = false;
bool RPGBuilder::MDD_iterative = false;
int  RPGBuilder::MDD_heuristic = 0;
bool RPGBuilder::MDD_eRPG = false;
...
SubproblemRPG::EvaluationInfo SubproblemRPG::getRelaxedPlan(MinimalState & theState, const double & maxPrefCostIn, const double & stateTS, const int & nextTIL, set<int> & goals, set<int> & goalFluents/*, set<int> & goalFluents*/, list<pair<int, VAL::time_spec> > & helpfulActions, list<pair<double, list<ActionAndHowManyTimes > > > & relaxedPlan, map<double, list<int> > * justApplied, double tilFrom) {
    ...
    // initial parameter indicating the #layers / 1000
    ...
    // clearing the hash table of reduced cost facts
    // key : int
    // value : double
    reducedCostFacts.clear();
    
    // copying the global variable initialUnsatisfiedProcessedStartPreconditions into our local one
    // these are initially the unsatisfy preconditions of all actions
	...
	if (evaluateDebug) {
        ...
        // looping throught all the numerical preconditions
		for (int i = 0; i < loopLim; ++i) {
			cout << i << " - ";
    ...
    const int vCount = theState.second.size(); // number of numeric variables
    const int avCount = RPGBuilder::getAVCount(); // number of additional variables created

    // initial fluent layer
    // key : double (#layers/1000)
    // values : vector<double>, values of the numeric variables
    // comparator : EpsilonComp
	...
	// Create an object of the MetricRPG class and MILPRPG
    ...
    // Computing the values of the auxiliar linear combination variables
	{
    ...
    // Initializing termination variables
	...
	// Iterators of goals
	const set<int>::iterator gsEnd = goals.end();		// propositional goals
	const set<int>::iterator gfEnd = goalFluents.end();	// numerical goals
    
    //Checking if are already satisfied
	{
    ...
    // Checking if the numerical goal are satisfy
    ...
    //Message if the goals are satisfy in the current state
    ...
    //Initialize varaibles of the SubProblem class
    ...
    *negativeNumericAchievedInLayer = (*negativeNumericAchievedInLayerReset);
    ...
    // Setting achieved propositional variables
	{
    ...
    // Setting achieved numerical variables
	{
		set<int>::iterator gfItr = goalFluents.begin();
...
//	const bool milpDebug = true;
	...
	while ((unsatisfiedGoals || unappearedEnds) && !factLayers.empty()) {
        ...
        if ((unsatisfiedGoals || unappearedEnds) && (milprpg.get() || metricrpg.get())) {
            ...
            //@mpcastro : creates layer of numerical variables --> fluentLayer
    		...
    		//@mpcastro : updates the layer with the numerical effects
            if (!alreadyAddedApplicableActions) {
                {
                    const int actLim = startPreconditionCounts.size();
                    for (int ae = 0; ae < actLim; ++ae) {
    		...
    		//@mpcastro : check if a numerical goals and preconditions is satisfy and stores the layer where it was satisfy
            for (int i = 0; i < loopLim; ++i) {
                double & currAIL = (*numericAchievedInLayer)[i];
                if (currAIL == -1.0) {
                    if ((*rpgNumericPreconditions)[i].isSatisfied(newTable)) {
...
//	evaluateDebug = true;
	...
	cout << "Printing return values: h = " << h << " lpGoalPrefViolation = " << lpGoalPrefViolation <<   " realGoalsSatisfied = " << realGoalsSatisfied << endl;
...
double SubproblemRPG::shiftFactLayersByLag(const double & currTS, const double & lag, map<double, FactLayerEntry, EpsilonComp > & factLayer, map<double, vector<double>, EpsilonComp > & fluentLayers, const bool excludeFirstLayer)
{
    ...
    const unsigned int aSize = factLayer.size();
...
void SubproblemRPG::processAnyCheaperFacts(const double & nlTime, double & maxLag)
{
    ...
    for (; recost != recostEnd; ++recost) {
        ...
        assert(currAct >= 0);
        assert((unsigned int) currAct < actionsToStartEffects->size());
...
void SubproblemRPG::updatePreferencesForFact(MinimalState & startState,
                                             const unsigned int & fID, const bool & isALiteral, const bool & polarity,
...
// Apply actions to a fact layer and return if it is applicable or not
...
void SubproblemRPG::findApplicableActions(MinimalState & theState, const int & nextTIL, list<pair<int, VAL::time_spec> > & applicableActions) {
	...
	//@mpcastro: check actions that doesn't have any preconditions
	{
        list<pair<int, VAL::time_spec> > & dependents = (RPGBuilder::getPreconditionlessPropositionalActions());
		list<pair<int, VAL::time_spec> >::iterator depItr = dependents.begin();
		const list<pair<int, VAL::time_spec> >::iterator depEnd = dependents.end();
	
		for (; depItr != depEnd; ++depItr) {
			const int currAct = depItr->first;
			//@mpcastro:  it ignores the action if it is a rogue action
			if (!RPGBuilder::rogueActions[currAct]) {
    ...
    // Iterate over the propositions of the state
	set<int>::iterator stateItr = theState.first.begin();
	const set<int>::iterator stateEnd = theState.first.end();

	for (; stateItr != stateEnd; ++stateItr) {
        // dependents = list of actions that have this precondition
		list<pair<int, VAL::time_spec> > & dependents = (*processedPreconditionsToActions)[*stateItr];

		list<pair<int, VAL::time_spec> >::iterator depItr = dependents.begin();
		const list<pair<int, VAL::time_spec> >::iterator depEnd = dependents.end();
        
        //@mpcastro: iterate over the actions and add to the list (toFilter) all the actions that fulfill the propositional requirements
		for (; depItr != depEnd; ++depItr) {
			const int currAct = depItr->first;
			if (!RPGBuilder::rogueActions[currAct]) {
				const VAL::time_spec startOrEnd = depItr->second;
				assert(startOrEnd == VAL::E_AT_START);
				{
					if (!(--startPreconditionCounts[currAct])) {
						bool mutex = false;
						{
							...
							//@mpcastro: iterate over the delete effects of the action to make sure is not mutex
							for (; dItr != dEnd; ++dItr) {
								if (theState.invariants.find((*dItr)->getStateID()) != theState.invariants.end()) {
									mutex = true;
									break;
...
//    //@ mpcastro: our code - start
//    for(unsigned int i = 0; i < startPreconditionCounts.size(); i++){
//        if(startPreconditionCounts[i] <= 0 && !RPGBuilder::rogueActions[i]){
//            toFilter.push_back(pair<int, VAL::time_spec>(i, VAL::E_AT_START));
//        }
//    }
//    //@mpcastro: our code - end

	list<pair<int, VAL::time_spec> >::iterator fItr = toFilter.begin();
	const list<pair<int, VAL::time_spec> >::iterator fEnd = toFilter.end();
    
    // iterate over the actions that satisfy their propositional preconditions
	for (; fItr != fEnd; ++fItr) {
		...
		if (isApplicable) {
            ...
            // iterate over numerical precondition to check applicability
			for (; npItr != npEnd; ++npItr) {
		...
		if (isApplicable) {
			if (fItr->second == VAL::E_AT_START) {
                if (RPGBuilder::isInteresting(fItr->first, theState.first, theState.startedActions) || RPGBuilder::planAnyway) {
