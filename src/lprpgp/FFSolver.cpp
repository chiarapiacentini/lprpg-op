...
int FF::statesExpanded = 0;

bool FF::firstImprover = false;

double FF::costPlan = 0.0;
double FF::costFeasible = DBL_MAX;
...
namespace CSBase {
	...
	int compareUnorderedSets(const unordered_set<int> & a, const unordered_set<int> & b, int nProp) { 						//@mpcastro: my implementation
		...
		for (int i=(nProp-1); i > 0; i--){
			if(a.find(i) == aEnd){ 					//element NOT in a
				if(b.find(i) != bEnd) return 1;		//element in b
			} else{									//element in a
				if(b.find(i) == bEnd) return -1;		//element NOT in b 
			}	
		}
...
//			if (av.stepID > bv.stepID) return -1;
	...
	int compareVecsMDD_min(const vector<double> & a, const vector<double> & b, const vector<double> & vmin){
		const int aSize = a.size();
		const int bSize = b.size();
		if (!aSize && !bSize) return 0;
		if (aSize < bSize) return 1;
		...
		for (int i = 0; i < aSize; ++i) {
			if (!ignorableFluents[i]) {
				const double av = a[i];
				const double bv = b[i];
				// only compare the values if one of the values is bigger than the minNeeded
				if (av > vmin[i] || bv > vmin[i]){
					if (av < bv) return 1;
					if (av > bv) return -1;
				}		
			}
	...
	int compareVecsMDD_max(const vector<double> & a, const vector<double> & b, const vector<double> & vmax){
		const int aSize = a.size();
		const int bSize = b.size();
		if (!aSize && !bSize) return 0;
		if (aSize < bSize) return 1;
...
public:
	...
	void printNewSolution()
	{
        ...
        cout << "\n; States evaluated: " << RPGBuilder::statesEvaluated << endl;                
        cout << ";\n;\n";
...
public:
	...
	void insert(SearchQueueItem* p, const int category=1) {
        ...
        for (int ilist = 1; ilist < 3; ++ilist) {
            ...
            if (FF::AStar && FF::skipEHC){
                //prim += p->plan.size();
                // heuristic plus cost of reaching the state
                prim += p->state.planCost;
            }
            ...
            if (q.empty()) {
                ...
                bool isFeasiblePlan = insHeuristic.getFoundAFeasiblePlan();
                if (isFeasiblePlan) cout << "Inserting feasible plan into the queue" << endl;
                
                //const double sec = !FF::AStar ? (ilist == 1 ? insHeuristic.second : insHeuristic.first) : -insHeuristic.first ;
                const double sec = !FF::AStar ? (ilist == 1 ? insHeuristic.second : insHeuristic.first) : (isFeasiblePlan ? 0 : insHeuristic.first );
                bool inserted = false;
...
FF::HTrio FF::evaluateStateWRTSchedule(SubproblemRPG* const rpg, ExtendedMinimalState & theState, set<int> & goals, set<int> & goalFluents, list<pair<int, VAL::time_spec> > & helpfulActions, list<pair<double, double> > & howMany, list<ScheduleNode*> & pointsOfConflict,list<FFEvent> & header, list<FFEvent> & now, FF::HTrio & bestNodeLimitHeuristic, list<FFEvent> *& bestNodeLimitPlan, bool & bestNodeLimitGoal, const int & spID, bool & stagnant, bool considerCache, map<double, list<int> > * justApplied, double tilFrom) {
    ...
    stringstream nameAction;
    if(!now.empty()){
        nameAction << *now.begin()->action;
    }

    theState.id = RPGBuilder::statesEvaluated;
    
    
    // check feasibility of relax plan
    if (FF::checkFeasiblePlan(rpg,relaxedPlan,theState)){
        cout << "Found feasible plan" << endl;
        HTrio htrio(h.first,h.second,h.first,h.goalState);
        htrio.setFoundAFeasiblePlan(true);
        htrio.setRelaxedPlan(relaxedPlan);
        
        if(costFeasible > (h.first + theState.planCost) ) {
            costFeasible = h.first + theState.planCost;
            cout << "New incumbent = " << costFeasible << endl;
        }
        
        DotSearchSpace::addNode(theState.id,nameAction.str().c_str(), 0, theState.idParent, rpg->comparisonHeuristic, theState.planCost);
        
        return htrio;
    }
    DotSearchSpace::addNode(theState.id,nameAction.str().c_str(), h.first, theState.idParent, rpg->comparisonHeuristic, theState.planCost);
...
void FF::applyActionToState(SubproblemRPG* const rpg, ExtendedMinimalState & theState, const int & actionToApply, const VAL::time_spec & ts) {
	...
	//Apply action -> numeric effects effects
    {
	...
	//Update the cost to reach the state and  plan length.
	theState.planCost += RPGBuilder::getActionCost()[actionToApply];
    ++(theState.planLength);
...
//@mpcastro my implementation to put the HA last
void reorderHelpfulLast(list<pair<int, VAL::time_spec> > & applicableActions, list<pair<int, VAL::time_spec> > & helpfulActions) {

	list<pair<int, VAL::time_spec> > tmp;
    tmp.swap(applicableActions);			//this one has all the actions and "applicable actions is empty"
	...
	//Iterate over applicable actions
	for (; itr != itrEnd; ++itr) {
		int w = 0;
		
		//Iterate over helful Actions until we find the one equal to the action
		// It also increase the weight... bigger weight is less important
		for (list<pair<int, VAL::time_spec> >::iterator hitr = hitrStart; hitr != hitrEnd; ++hitr) {
			if (itr->first == hitr->first && itr->second == hitr->second) {
				break;
			}
			w++;
		...
		for (; aaItr != aaEnd && (w <= *wItr); ++aaItr, ++wItr); //finds the possition to put it with respect to the weight
...
bool FF::checkFeasiblePlan(SubproblemRPG * const rpg,list<pair<double, list<ActionAndHowManyTimes> > > & relaxedPlan, ExtendedMinimalState & theState)
{
    //@mpcastro: our own function to check if a RelaxedPlan is a valid plan
    
    if (relaxedPlan.empty()){
//        cout << "empty relaxed plan" << endl;
        return false;
    }
    //cout << "not empty relaxed plan" << endl;

    ExtendedMinimalState currState = theState;
    
    // Check if actions are applicable
    for (auto & planStep : relaxedPlan){
        for(auto & action : planStep.second){
            int actionID = action.first->getID();
                
            if ( rpg->testApplicability(currState, currState.nextTIL, make_pair(actionID,VAL::E_AT_START), false,false)) {
                // Apply the action
                //cout << "Action is applicable" << endl;
                applyActionToState(rpg, currState, actionID, VAL::E_AT_START);
            }
            else{
                //not applycable
                //cout << "Action is not applicable" << endl;
                return false;
            }
    ...
    cout << "All actions are applicable" << endl;
    
    //Check if resulting state is a goal state
    return checkGoalState(currState);;
}
    
bool FF::checkGoalState(ExtendedMinimalState & theState){
    ...
    //Propositional goals
    {
        ...
        list<Literal*> & checkFor = RPGBuilder::getLiteralGoals();
    ...
    //Numeric goals
    list<RPGBuilder::NumericPrecondition> & checkFor = RPGBuilder::getNumericGoals();
    list<RPGBuilder::NumericPrecondition>::iterator fItr = checkFor.begin();
    const list<RPGBuilder::NumericPrecondition>::iterator fEnd = checkFor.end();
    
    
    for (; fItr != fEnd; ++fItr) {

        if ( !fItr->isSatisfied(theState.second) ) {
                return false;
        }
        
    }
    
    //cout << "Final state is a goal state" << endl;
    
    return true;
    
}

    bool FF::checkGoalState(MinimalState & theState){
...
list<FFEvent> * FF::solveSubproblemWRTSchedule(LiteralSet & startingState, vector<double> & startingFluents, SubProblem* const toSolve, const int nodeLimit,bool & reachedGoal, const int & spID, list<FFEvent> * oldSoln, double & oldSolutionValue, int & relaxedStepCount) {    
	...
	// we skip this will lop if the are using BFS or AStar
            ...
            for (int sc = 0; sc < 2; ++sc) {
				...
				if (zvsItr->second.empty()) {
					...
					if (succ->heuristicValue.first != -1.0) {
						...
						//State safisty the goal conditions
...
//			printState(succ->state);
			...
			double maxIteration = 1.0;
			if (FF::multipleHelpfuls) {
				maxIteration = hhmItr->first;
			}

			for (double hm = 0; hm < maxIteration; hm += 1.0) {
				if (hm > 0.0) {
...
//					succ->printPlan();
	...
	//@mpcastro: start search procedure
	while (!searchQueue.empty()) {
        ...
        //@mpcastro: get next state to expand
		auto_ptr<SearchQueueItem> currSQI(searchQueue.pop_front(nextPopFrom));
        FF::statesExpanded++;
		...
		//@mpcastro: check if the state is a goal state
		if (currSQI->heuristicValue.goalsSatisfied) {
            bool roomforimprovement = true;                                      
			const double pcost = PreferenceHandler::getCurrentCost(currSQI->state);
            if (pcost < bestSolutionPCost) {
            	cout << "g[" << currSQI->state.cost + pcost << "] "; cout.flush();
                        
                reachedGoal = true;                                
                relaxedStepCount = 0;
                delete bestSolution;
                currSQI->printNewSolution();
                bestSolution = new list<FFEvent>(currSQI->plan);
                bestSolutionCost = currSQI->state.cost + pcost;
                bestSolutionPCost = pcost;
                costPlan = currSQI->state.planCost;
                if (pcost == 0.0) return bestSolution;
                               
                bestHeuristic.first = searchQueue.newBestPrefScore(bestSolutionPCost);
         	}
         	roomforimprovement = (currSQI->heuristicValue.second < bestSolutionPCost);
                            
         	if (roomforimprovement) {
         		cout << "^"; cout.flush();
			} 
		}
        //@mpcastro: check if state is a feasible solution
        if(currSQI->heuristicValue.getFoundAFeasiblePlan()) {
            
            const double pcost = PreferenceHandler::getCurrentCost(currSQI->state);
            cout << "f[" << currSQI->state.cost + pcost << "] "; cout.flush();
            
            // Create plan
            delete bestSolution;
            bestSolution = new list<FFEvent>(currSQI->plan);
            
            for (auto& planStep: currSQI->heuristicValue.getRelaxedPlan() ){
                for(auto& actions: planStep.second){
                    bestSolution->push_back(FFEvent(actions.first, 1, 1) );
                }
            
            }
            
            bestSolutionPCost = pcost;
            costPlan = currSQI->state.planCost + currSQI->heuristicValue.first;
		...
		//@mpcastro: Find applicable actions to the state
		...
		reorderHelpfulLast(applicableActions, currSQI->helpfulActions);  //@ my implemenation
		...
		for (;helpfulActsItr != helpfulActsEnd; ++helpfulActsItr) {
            ...
            if (helpfulActsItr->first <= -1) {
            	...
            	//@mpcastro: apply action to current state to get succesor
            ...
            
            {
				...
				//@mpcastro: checks if state has been visited before or not
                if (zvsItr->second.empty()) {
                	//Case 1: new state
                    visitTheState = 1;
				} 
				else {
                    //Case 2: state visited before... we need to compare its value
                    //Case 2.1: check if the cost to reach the state is less or not --> re-open the state
                    visitTheState = paretoCheck(zvsItr->second, succ->state.preferenceStatus);
                    //if (FF::AStar && succ->state.planLength < zvsItr->first.planLength) {
                    if (FF::AStar && succ->state.planCost < zvsItr->first.planCost) {			//consider plan cost to see if a state should be open
                        visitTheState = 1;
                        // now put this in the node
                        zealousVisitedStates.erase(zvsItr);
                        zvsItr = zealousVisitedStates.insert(make_pair(succ->state, list<vector<AutomatonPosition> >())).first;
                        visitedStates.erase(vsItr);
                        vsItr = visitedStates.insert(make_pair(succ->state, list<vector<AutomatonPosition> >())).first;
                        
                        if (GlobalSchedule::globalVerbosity & 2) cout << "re-opening node " << endl;

                    }
                    if (visitTheState == 0) {
                    	//Case 2.2: state duplicated with higher cost
                        if (GlobalSchedule::globalVerbosity & 2) {
				...
				if (visitTheState) {
                    ...
                    if (helpfulActsItr->first >= 0) {
                    	...
                    	//@mpcastro: computes heurisitic value of the state
                        list<FFEvent> tEvent;
                        succ->heuristicValue = evaluateStateWRTSchedule(toSolve->rpg, succ->state, goals, numericGoals, succ->helpfulActions, succ->helpfulHowManyTimes, succ->pointsOfConflict,succ->plan, tEvent, bestNodeLimitHeuristic, bestNodeLimitPlan, reachedGoal, spID, stagnant);
                        //cout << "State feasible = " << succ->heuristicValue.getFoundAFeasiblePlan() << endl;
                    }
					//}
	
					//succ->printPlan();
					//@mpcastro: In case the state is feasible
					if (succ->heuristicValue.first != -1.0) {
                        ...
                        /*
                        // @mpcastro: checks if the state satisfies the goals
                        if (succ->heuristicValue.goalsSatisfied) {
                        ...
                        */
                        bool biggerThanFeasible = false;
                        if( costFeasible < succ->heuristicValue.first + succ->state.planCost){
                            biggerThanFeasible = true;
                        }
                        ...
                        if (roomforimprovement) {
                            ...
                            if(biggerThanFeasible){
                                if (GlobalSchedule::globalVerbosity & 2) {
                                    cout << "\tWorst than incumbent = " << costFeasible << "\n";
                                } else {
                                    cout << "w"; cout.flush();
                                }
