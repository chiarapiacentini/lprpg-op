...
void IPStateBasedLinearModel::buildConstraints(){
...
void IPStateBasedLinearModel::initialiseVariables(){
...
void IPStateBasedLinearModel::addVariables(int tMin, int tMax){
...
//void IPStateBasedLinearModel::numericPreconditions(int tMin, int tMax){
...
//                    toAdd.push_back({IPBoolean, indexAction[{RPGBuilder::getInstantiatedOp(iA),t}], infinity});
//                    toAdd.push_back({IPContinous, indexPNE[{RPGBuilder::getPNE(lhs), t}], -1});
//                    solver->addRow(toAdd, IPLess, -epsilon - rhs + infinity);
//                    
...
//                        epsilon-=0.000000001;
//                    }
//                    toSum.push_back({IPBoolean, indexAction[{RPGBuilder::getInstantiatedOp(iA),t}], infinity});
//                    solver->addRow(toSum,IPLess, epsilon + constant - rhs + infinity);
//                }
//            }
//        }
//    }
...
void IPStateBasedLinearModel::numericEffects(int tMin, int tMax){
...
void IPStateBasedLinearModel::numericInitialStateConstraint(){
...
void IPStateBasedLinearModel::numericGoalStateConstraint(){
...
void IPStateBasedLinearModel::numericNoopConstraint(int tMin, int tMax){
    //bool debug = true;
...
//void IPStateBasedLinearModel::numericNoopEffects(int tMin, int tMax){
...
//                toAdd.push_back({IPBoolean, indexPNENoop[{p,t-1}], infinity});
//                cout << *RPGBuilder::getPNE(iV) << " " << t << " " << minMaxValues[t][iV].first << " " << minMaxValues[t][iV].second << endl;
//                solver->addRow(toAdd, IPLess,infinity);
//            }
//
//            {
//                list<IPSolver::IPConstraint> toAdd;
...
//                toAdd.push_back({IPContinous, indexPNE[{p,t-1}], -1});
//                toAdd.push_back({IPBoolean, indexPNENoop[{p,t-1}], -infinity});
//                solver->addRow(toAdd, IPGreater,-infinity);
//            }
//
//        }
//    }
...
void IPStateBasedLinearModel::numericMutex(int tMin, int tMax){
...
void IPStateBasedLinearModel::numericMutexEffects(int tMin, int tMax){
...
void IPStateBasedLinearModel::incrementTimeHorizon(MinimalState & state, int increment ){
    int start = maxT;
...
//
//void IPStateBasedLinearModel::extractSolution(){
